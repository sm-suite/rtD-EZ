...
def pos():
	...
	def AniOverview(i):
		...
		Lret = [b1w, s1w]
		LL = [b1w, b1r, s1r, s1w]
		LLabel = 'buy-return', 'buy-risk','sell-risk', 'sell-return'
		labels='buy-Risk', 'sell-Risk'
		...
		ax3.clear()
		ax3.step(viz_rng, b1, color='#009900')
		ax3.step(viz_rng, s1, color='#FF0000')
...
def risk():
	...
	def AniOverview(i):
		...
		ax4.clear()
		ax4.pie(Lrisk, labels=['buy-risk', 'sell-risk'], colors=['#FF8000', '#9999FF'], autopct='%.0f%%')

		
		ax5.clear()
		ax5.pie(Lret, labels=['buy-return', 'sell-return'], colors=['#FF8000', '#9999FF'], autopct='%.0f%%')
